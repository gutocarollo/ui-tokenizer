...
# Walk the registry: prune dead lines, keep live ones, and adopt the first
# live entry. Rewrites the file only when something actually changed, so a
# healthy registry is never touched.
