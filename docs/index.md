...
| [`pending/`](pending/README.md) | **o backlog como artefato** — uma pendência = um arquivo, resolver é apagar. `pending/index.md` é gerado e `pending/README.md` é o contrato; o guard confere que o ponteiro de cada item ainda aponta para o que ele diz. Contrato constitucional em `SCHEMA.md` §3.1 | living |
...
| doc | o que é | status |
|---|---|---|
| [`law/GRAMMAR.md`](law/GRAMMAR.md) | a gramática de naming: vocabulário fechado de entidades, anatomias, propriedades, variantes e estados. **Cinco** palavras banidas — `surface`, `semantic`, `content`, `label`, `foreground` (§3.1); a anatomia é `text` e a tinta de uma parte é `color` (§5.6, trocadas em 2026-08-01) | living |
